...
void update_user_movie_matrix(int user_start_idx, int movie_start_idx, 
    ...
    // test output
    //printf("The user start idx %d, the movie start idx %d, block rating num %d\n", 
    //        user_start_idx, movie_start_idx, block_rating_num);
}
...
#pragma offload target(mic) \
...
#endif
  {
    ...
    /* merge the merge_factor consecutive updates together */
    int merge_factor = 1;
    if (num_of_threads < 16) {
        merge_factor = 1;
    } else if (num_of_threads == 32) {
        merge_factor = 8;
    } else {
        merge_factor = 16;
    }
    ...
    /* start optimization */
    auto optimize_start = Clock::now();
    double optimize_time = 0;
    for (int out_iter = 0; out_iter < iters; ++out_iter) {

        /* start subroutine */
        auto subroutine_start = Clock::now();
        double subroutine_time = 0;
        for (int iter = 0; iter < num_of_threads / merge_factor; ++iter) {

            auto allocate_start = Clock::now();
            double allocate_time = 0;

            // allocate independent blocks to different threads
            std::vector<int> user_start_idx(num_of_threads * merge_factor, 0);
            std::vector<int> movie_start_idx(num_of_threads * merge_factor, 0);
            std::vector<int> user_end_idx(num_of_threads * merge_factor, 0);
            std::vector<int> movie_end_idx(num_of_threads * merge_factor, 0);

            for (int merge_iter = 0; merge_iter < merge_factor; ++merge_iter) {
                for (int idx = 0; idx < num_of_threads; ++idx) {
                    int block_idx = (idx + iter * merge_factor + merge_iter) % num_of_threads;
                    
                    int total_idx = merge_iter * num_of_threads + idx;
                    user_start_idx[total_idx] = std::min(idx * user_span, user_num);
                    user_end_idx[total_idx] = std::min((idx + 1) * user_span, user_num);

                    movie_start_idx[total_idx] = std::min(block_idx * movie_span, movie_num);
                    movie_end_idx[total_idx] = std::min((block_idx + 1) * movie_span, movie_num);
                }
            }

            allocate_time += duration_cast<dsec>(Clock::now() - allocate_start).count();
            ...
            for (tidx = 0; tidx < num_of_threads * merge_factor; ++tidx) {
            ...
            /* ========end parapllel========= */
            /* ============================== */
            parallel_time += duration_cast<dsec>(Clock::now() - parallel_start).count();
            //printf("Parallel Time: %lf.\n", parallel_time);  
        }

        subroutine_time += duration_cast<dsec>(Clock::now() - subroutine_start).count();
        //printf("outiter %d, subroutine Time : %lf.\n", out_iter, subroutine_time);  
    ...
    optimize_time += duration_cast<dsec>(Clock::now() - optimize_start).count();
    printf("optimize Time (including all iterations): %lf.\n", optimize_time);  
  }
